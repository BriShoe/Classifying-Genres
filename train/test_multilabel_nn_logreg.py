...
if __name__ == '__main__':
    ...
    y_predicts = np.empty(shape=(len(full_train), num_genres))
    y_truths = np.empty(shape=(len(full_train), num_genres))
    row = 0
    for fold, (train_idx,
        ...
        model = multi_classifier(
            len(full_train.columns) - num_genres, num_genres)
    ...
    # save model
    torch.save(model.state_dict(), f"models/neuralnetworks/nn_logreg_{}")

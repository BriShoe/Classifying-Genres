...
if __name__ == '__main__':
    ...
    X = full_train.iloc[:, :len(full_train.columns) - num_genres]
    isomap = fitIsomap(X, optimalfeatures)
    X = pd.DataFrame(isomap.transform(X))
    ...
    with open(f"../models/neuralnetworks/nn_isomap_{optimalfeatures}features.txt", "a") as f:
        f.write(f"Number of Features: {optimalfeatures} \nBatch Size: {batchsize} \nEpochs: {epochs} \nLearning Rate: {learningrate} \nNeurons: {neurons}")
    torch.save(model.state_dict(), f"../models/neuralnetworks/nn_isomap_{optimalfeatures}features")
